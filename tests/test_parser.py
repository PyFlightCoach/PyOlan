...
from tuning.builders.IAC.manbuilder import iacmb
from tuning.builders.IMAC.manbuilder import imacmb

from pyolan.parser import ParsedOlanFig, parse_olan


def test_entry_direction():
    ofigs = parse_olan("13% -``5if```,4ao(,1),22+`", imacmb)
...
def test_create_unl_loop_definition():
    ...
    ofig: ParsedOlanFig = parse_olan("`+o34;5f+`", imacmb)[0]
...
def test_turn():
    fig: ParsedOlanFig = parse_olan("4j", imacmb)[0]
...
def test_tailslide():
    fig: ParsedOlanFig = parse_olan("~~.,3ita'3....~>", iacmb)
